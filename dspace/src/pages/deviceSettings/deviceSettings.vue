<template>
  <view class="page">
    ...
    <view class="section">
      ...
      <view class="setting-card">
        ...
        <view class="setting-item">
          ...
          <input class="setting-input" v-model="levelTime" placeholder="1" />
    ...
    <view class="section">
      <view class="section-title">
        <view class="title-bar"></view>
        <text class="title-text">温度开关设置</text>
      </view>

      <view class="switch-grid">
        <view class="switch-item">
          <view class="switch-label">温度开关1</view>
          <MySwitch v-model="tof1Switch" />
        </view>
        <view class="switch-item">
          <view class="switch-label">温度开关2</view>
          <MySwitch v-model="tof2Switch" />
        </view>
        <view class="switch-item">
          <view class="switch-label">温度开关3</view>
          <MySwitch v-model="tof3Switch" />
        </view>
        <view class="switch-item">
          <view class="switch-label">温度开关4</view>
          <MySwitch v-model="tof4Switch" />
        </view>
      </view>
    </view>
    ...
    <view class="section">
      ...
      <view class="alarm-method-card">
        <view class="method-options">
          ...
          <RadioGroup
            :value="dialingMethod"
            :options="dialingMethodOptions"
            @change="dialingMethodChange"
          />
        </view>
...
import { parseValueFromEvent } from "@/utils/tools.js"
...
export default {
    ...
    phoneNumbers: {
      get() {
        if (this.deviceInfo.alarmPhones) {
          return Array.isArray(this.deviceInfo.alarmPhones)
            ? this.deviceInfo.alarmPhones
            : [""]
        }
        return [""]
      },
  ...
  methods: {
    ...
    alarmMethodChange(e) {
      this.alarmMethod = parseValueFromEvent(e)
    },
    // 更新单个电话号码
      ...
      uni.showModal({
        ...
        success: (res) => {
          if (res.confirm) {
            ...
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "levelTime",
              value: this.defaultValue.levelTime,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "gasUpperLimit",
              value: this.defaultValue.gasUpperLimit,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "gasLowerLimit",
              value: this.defaultValue.gasLowerLimit,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "tempUpperLimit",
              value: this.defaultValue.tempUpperLimit,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "tempLowerLimit",
              value: this.defaultValue.tempLowerLimit,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "humidityUpperLimit",
              value: this.defaultValue.humidityUpperLimit,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "humidityLowerLimit",
              value: this.defaultValue.humidityLowerLimit,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "tof1",
              value: this.defaultValue.tof1,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "tof2",
              value: this.defaultValue.tof2,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "tof3",
              value: this.defaultValue.tof3,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "tof4",
            ...
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "dialingMethod",
              value: this.defaultValue.dialingMethod,
            })
            this.$store.commit("deviceDetail/UPDATE_DEVICE_FIELD", {
              field: "alarmMethod",
              value: this.defaultValue.alarmMethod,
            })
    ...
    async saveSettings() {
      ...
      if (
        ...
        this.deviceInfo.deviceName.trim() === ""
      ) {
        uni.showToast({
          title: "设备名称不能为空",
          icon: "none",
      ...
      if (
        this.deviceInfo.alarmPhones &&
        Array.isArray(this.deviceInfo.alarmPhones) &&
        this.deviceInfo.alarmPhones.length > 0
      ) {
        const phoneReg = /^1[3-9]\d{9}$/
        const invalidPhones = this.deviceInfo.alarmPhones.filter(
          (phone) => phone && phone.trim() && !phoneReg.test(phone)
        )
      ...
      if (
        this.deviceInfo.gasUpperLimit !== undefined &&
        this.deviceInfo.gasUpperLimit !== null &&
        this.deviceInfo.gasLowerLimit !== undefined &&
        this.deviceInfo.gasLowerLimit !== null
      ) {
      ...
      if (
        this.deviceInfo.tempUpperLimit !== undefined &&
        this.deviceInfo.tempUpperLimit !== null &&
        this.deviceInfo.tempLowerLimit !== undefined &&
        this.deviceInfo.tempLowerLimit !== null
      ) {
      ...
      if (
        this.deviceInfo.humidityUpperLimit !== undefined &&
        this.deviceInfo.humidityUpperLimit !== null &&
        this.deviceInfo.humidityLowerLimit !== undefined &&
        this.deviceInfo.humidityLowerLimit !== null
      ) {
        if (
          this.deviceInfo.humidityUpperLimit <=
          this.deviceInfo.humidityLowerLimit
        ) {
          uni.showModal({
            title: "数值范围错误",
      ...
      // 校验阶梯时间必须大于等于0
      if (
        this.deviceInfo.levelTime !== undefined &&
        this.deviceInfo.levelTime !== null &&
        this.deviceInfo.levelTime < 0
      ) {
        uni.showModal({
          title: "数值范围错误",
          content: "阶梯时间必须大于等于0",
      ...
      try {
        ...
        if (
          this.deviceInfo.tof1 !== undefined &&
          this.deviceInfo.tof1 !== null
        ) {
          requestData.tof1 = this.deviceInfo.tof1
        }
        if (
          this.deviceInfo.tof2 !== undefined &&
          this.deviceInfo.tof2 !== null
        ) {
          requestData.tof2 = this.deviceInfo.tof2
        }
        if (
          this.deviceInfo.tof3 !== undefined &&
          this.deviceInfo.tof3 !== null
        ) {
          requestData.tof3 = this.deviceInfo.tof3
        }
        if (
          this.deviceInfo.tof4 !== undefined &&
        ...
        if (
          ...
          this.deviceInfo.dialingMethod !== null
        ) {
          requestData.dialingMethod = this.deviceInfo.dialingMethod
        }
        if (
          this.deviceInfo.alarmMethod !== undefined &&
          this.deviceInfo.alarmMethod !== null
        ) {
          requestData.alarmMethod = this.deviceInfo.alarmMethod
        }
        if (
          this.deviceInfo.alarmPhones !== undefined &&
          this.deviceInfo.alarmPhones !== null &&
          Array.isArray(this.deviceInfo.alarmPhones) &&
          this.deviceInfo.alarmPhones.length > 0
        ) {
